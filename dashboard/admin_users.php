            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr>
                            <th class="py-3 px-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest"># ID</th>
...
async function loadData() {
    try {
        ...
        document.getElementById('farmersTable').innerHTML = '<tr><td colspan="8" class="py-8 text-center text-red-400">Failed to load farmer data.</td></tr>';
    }
}
...
function renderFarmers(farmers) {
    ...
    tbody.innerHTML = farmers.map((u) => `
        <tr class="border-t border-slate-50 hover:bg-slate-50 transition-colors">
            <td class="py-4 px-6 text-xs text-slate-400 font-bold">#F${String(u.id).padStart(4,'0')}</td>
            <td class="py-4 px-6">
                <div class="flex items-center gap-3">
